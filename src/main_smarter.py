...
def eliminate_corners(inp, dir, y, x, domains, Num_of_domain_values, domain_levels, assigns):
    ...
    if(dir == '┌'):
        ...
        for k in range(length):
            if(inp[i][j] != '_'):
                break
            reduce_domain([dir], (i, j), domains, Num_of_domain_values, domain_levels, assigns)
            i -= 1
            j -= 1
        return
...
def initial_corner_constrains(inp, domains, Num_of_domain_values, domain_levels, is_in_domain):
    ...
    for i in range(1, w-1):
        #domain[(0, i)] = {'┌', '┐', '─'}
        if(is_in_domain[(0, i)]):
            eliminate_corners(inp, '└', 0, i, domains, Num_of_domain_values, domain_levels, [])
            eliminate_corners(inp, '┘', 0, i, domains, Num_of_domain_values, domain_levels, [])
    ...
    for i in range(1, h-1):
        #domain[(i, 0)] = {'└', '┌', '│'}
        if(is_in_domain[(i, 0)]):
            eliminate_corners(inp, '┐', i, 0, domains, Num_of_domain_values, domain_levels, [])
            eliminate_corners(inp, '┘', i, 0, domains, Num_of_domain_values, domain_levels, [])

    for i in range(1, w-1):
        #domain[(w-1, i)] = {'└', '┘', '─'}
        if(is_in_domain[(w-1, i)]):
            eliminate_corners(inp, '┐', w-1, i, domains, Num_of_domain_values, domain_levels, [])
            eliminate_corners(inp, '┌', w-1, i, domains, Num_of_domain_values, domain_levels, [])
    ...
    for i in range(1, h-1):
        ...
        if(is_in_domain[(i, w-1)]):
            eliminate_corners(inp, '└', i, w-1, domains, Num_of_domain_values, domain_levels, [])
            eliminate_corners(inp, '┌', i, w-1, domains, Num_of_domain_values, domain_levels, [])
...
def delete_corner(inp, corner_values, location, domains, Num_of_domain_values, domain_levels, assigns):
    for value in corner_values:
        if(value in domains[location]):
            reduce_domain([value], location, domains, Num_of_domain_values, domain_levels, assigns)
            eliminate_corners(inp, value, location[0], location[1], domains, Num_of_domain_values, domain_levels, assigns)
...
def reduct_domain(inp, dir, location, domains, Num_of_domain_values, domain_levels, assigns):
    for value in ['└', '┌', '│', '┘', '─', '┐']:
        if(value == dir):
            continue
        if(value != '│' and value != '─'):
            delete_corner(inp, [value], location, domains, Num_of_domain_values, domain_levels, assigns)
        else:
            reduce_domain([value], location, domains, Num_of_domain_values, domain_levels, assigns)
    ...
    for terminal in terminals:
        ...
        if(len(terminal_domain[terminal]) == 1):
            ...
            if(m == 1 and is_in_domain[(i-1, j)]):
                ...
                delete_corner(inp, ['┌', '└'], (i, j+1), domains, Num_of_domain_values, domain_levels, [])
...
def reduce_terminals(inp, m, location, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns):
    ...
    if(len(terminal_domain[location]) == 1):
            ...
            if(m == 1):
                delete_corner(inp, ['┘', '└'], (i-1, j), domains, Num_of_domain_values, domain_levels, assigns)
                reduce_domain(['─'], (i-1, j), domains, Num_of_domain_values, domain_levels, assigns)
                ...
                delete_corner(inp, ['┌', '└'], (i, j+1), domains, Num_of_domain_values, domain_levels, assigns)
                reduce_domain(['│'], (i, j+1), domains, Num_of_domain_values, domain_levels, assigns)
    ...
    for i in range(1, w-1):
        ...
        if(is_in_domain[(0, i)]):
            if(is_in_domain[(0, i-1)] and '┌' not in domains[(0, i)]):
                delete_corner(inp, ['┐'], (0, i-1), domains, Num_of_domain_values, domain_levels, [])
            if(is_in_domain[(0, i+1)] and '┐' not in domains[(0, i)]):    
                delete_corner(inp, ['┌'], (0, i+1), domains, Num_of_domain_values, domain_levels, [])
    ...
    for i in range(1, h-1):
        ...
        if(is_in_domain[(i, 0)]):
            if(is_in_domain[(i-1, 0)] and '┌' not in domains[(i, 0)]):
                delete_corner(inp, ['└'], (i-1, 0), domains, Num_of_domain_values, domain_levels, [])
            if(is_in_domain[(i+1, 0)] and '└' not in domains[(i, 0)]):    
                delete_corner(inp, ['┌'], (i+1, 0), domains, Num_of_domain_values, domain_levels, [])
    ...
    for i in range(1, w-1):
        ...
        if(is_in_domain[(w-1, i)]):
            if(is_in_domain[(w-1, i-1)] and '└' not in domains[(w-1, i)]):
                delete_corner(inp, ['┘'], (w-1, i-1), domains, Num_of_domain_values, domain_levels, [])
            if(is_in_domain[(w-1, i+1)] and '┘' not in domains[(w-1, i)]):    
                delete_corner(inp, ['└'], (w-1, i+1), domains, Num_of_domain_values, domain_levels, [])
    ...
    for i in range(1, h-1):
        ...
        if(is_in_domain[(i, h-1)]):
            if(is_in_domain[(i-1, h-1)] and '┐' not in domains[(i, h-1)]):
                delete_corner(inp, ['┘'], (i-1, h-1), domains, Num_of_domain_values, domain_levels, [])
            if(is_in_domain[(i+1, h-1)] and '┘' not in domains[(i, h-1)]):    
                delete_corner(inp, ['┐'], (i+1, h-1), domains, Num_of_domain_values, domain_levels, [])
        ...
        if(_next not in assigns):
            ...
            if(is_in_terminals[(i-1, j)]):
                if(color != colors[i-1][j]):
                    delete_corner(inp, ['┘', '└'], (i, j), domains, Num_of_domain_values, domain_levels, assigns)
                    reduce_domain(['│'], (i, j), domains, Num_of_domain_values, domain_levels, assigns)
...
def Assign(inp, dir, location, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals):
    ...
    if(dir == '│'):
        ...
        if(is_in_domain[(i, j+1)]):
            ...
            reduce_terminals(inp, 4, (i, j+1), terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns)
        ...
        if(is_in_domain[(i, j-1)]):
            ...
            reduce_terminals(inp, 3, (i, j-1), terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns)

        if(is_in_domain[(i+1, j)]):
            delete_corner(inp, ['┌', '┐'], (i+1, j), domains, Num_of_domain_values, domain_levels, assigns)
            reduce_domain(['─'], (i+1, j), domains, Num_of_domain_values, domain_levels, assigns)
        elif(is_in_terminals[(i+1, j)]):
            reduce_terminals_domain(inp, (i, j), (i+1, j), domains, Num_of_domain_values, domain_levels, assigns, is_in_domain)
        ...
        if(is_in_domain[(i+1, j+1)]):
            delete_corner(inp, ['┘'], (i+1, j+1), domains, Num_of_domain_values, domain_levels, assigns)
        ...
        if(colors[i-1][j] != '_' and colors[i+1][j] == '_'):
            color_path(inp, (i, j), (i-1, j), domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_terminals)
        ...
        if(is_in_domain[(i, j+1)]):
            ...
            reduce_terminals_domain(inp, (i, j), (i, j+1), domains, Num_of_domain_values, domain_levels, assigns, is_in_domain)
        ...
        if(is_in_domain[(i+1, j)]):
            ...
            reduce_terminals(inp, 1, (i+1, j), terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns)
        
        if(is_in_domain[(i-1, j)]):
            delete_corner(inp, ['┌', '┐'], (i-1, j), domains, Num_of_domain_values, domain_levels, assigns)
            reduce_domain(['│'], (i-1, j), domains, Num_of_domain_values, domain_levels, assigns)
        elif(is_in_terminals[(i-1, j)]):
            reduce_terminals(inp, 2, (i-1, j), terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns)
        ...
        if(colors[i][j-1] != '_' and colors[i][j+1] == '_'):
            color_path(inp, (i, j), (i, j-1), domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_terminals)
        ...
        if(is_in_domain[(i+1, j+1)]):
            reduce_domain('┘', (i+1, j+1), domains, Num_of_domain_values, domain_levels, assigns)
...
def apply_arc_consistency(inp, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals):
    ...
    while(len(domain_levels[0])):
        ...
        Assign(inp, dire, loc, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals)
...
def backtrack(inp, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals):
    ...
    Assign(inp, dire, loc, terminals, _terminal_domain, _domains, _Num_of_domain_values, _domain_levels, _assigns, _colors, is_in_domain, is_in_terminals)
    is_consistant = apply_arc_consistency(inp, terminals, _terminal_domain, _domains, _Num_of_domain_values, _domain_levels, _assigns, _colors, is_in_domain, is_in_terminals)
    
    if(is_consistant):
        res = backtrack(inp, terminals, _terminal_domain, _domains, _Num_of_domain_values, _domain_levels, _assigns, _colors, is_in_domain, is_in_terminals)
        if(res != False):
            return res
    ...
    Assign(inp, dire, loc, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals)
    iss_consistant = apply_arc_consistency(inp, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals)

    if(iss_consistant):
        _res = backtrack(inp, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals)
        if(_res != False):
            return _res
...
def solve(path):
    ...
    domains, Num_of_domain_values, domain_levels, terminals, is_in_domain, is_in_terminals = get_initial_domains(inp)

    print('number of variables: ', len(domains))
    colors = copy.deepcopy(inp)

    initial_corner_constrains(inp, domains, Num_of_domain_values, domain_levels, is_in_domain)

    terminal_domain = get_terminals_domain(inp, terminals, domains, Num_of_domain_values, domain_levels, is_in_domain)

    apply_terminals_constrains(inp, terminals, domains, Num_of_domain_values, domain_levels, is_in_domain, is_in_terminals)

    reduce_borders_domain(inp, domains, Num_of_domain_values, domain_levels, is_in_domain)
    ...
    Assigns = backtrack(inp, terminals, terminal_domain, domains, Num_of_domain_values, domain_levels, assigns, colors, is_in_domain, is_in_terminals)
...
solve('../input/input55.txt')
solve('../input/input77.txt')
solve('../input/input88.txt')
solve('../input/input991.txt')
solve('../input/input10101.txt')
solve('../input/input10102.txt')
solve('../input/input1212.txt')
solve('../input/input1214.txt')
solve('../input/input1414.txt')
